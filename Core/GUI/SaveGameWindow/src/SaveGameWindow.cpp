...
void SaveGameWindow::show(SDL_Window * window, SDL_Renderer * renderer) {
    ...
    gui::MenuItem menuItems[] = {
            { "CreateSave", gui::GuiButtons::CreateSave,      { 300, 200, 200, 50 } },
                    ...
                    for (int i = 0; i < numMenuItems; i++) {
                        if (mouseX >= menuItems[i].pos.x && mouseX <= menuItems[i].pos.x + menuItems[i].pos.w &&
                            ...
                            switch (menuItems[i].button) {
                                ...
                                case gui::GuiButtons::CreateSave:
                                {
                                    quit = true;
                                    m_stateWindow = StateWindow::CreateSave ;
                                    break;
                                }
...
SaveGameWindow::StateWindow SaveGameWindow::stateWindow() const {
