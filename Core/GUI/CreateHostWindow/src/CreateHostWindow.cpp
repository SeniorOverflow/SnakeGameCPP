...
CreateHostWindow::CreateHostWindow()
{

    std::cout<<"Start  window"<<std::endl;
}

void CreateHostWindow::show(SDL_Window * window, SDL_Renderer * renderer) {
    ...
    gui::MenuItem menuItems[] = {
            { "CreateHostStart", gui::GuiButtons::CreateHostStart,      { 300, 200, 200, 50 } },
                    ...
                    for (int i = 0; i < numMenuItems; i++) {
                        if (mouseX >= menuItems[i].pos.x && mouseX <= menuItems[i].pos.x + menuItems[i].pos.w &&
                            ...
                            switch (menuItems[i].button) {
                                ...
                                case gui::GuiButtons::CreateHostStart:
                                {
                                    quit = true;
                                    m_stateWindow = StateWindow::CreateHostStart ;
                                    break;
                                }
...
CreateHostWindow::StateWindow CreateHostWindow::stateWindow() const {
